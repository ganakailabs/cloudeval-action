...
- The install step runs the official CLI installer over HTTPS and falls back to `@ganakailabs/cloudeval-cli` from npm if the installer endpoint is unavailable; pin `cli_install_url` or set `skip_cli_install` and supply `cloudeval` on `PATH` if your policy requires a vetted binary.
