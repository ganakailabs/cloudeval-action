...
Access keys are intentionally time-limited and are **not renewed in place**. When a key expires, create a new GitHub Actions CI key, replace the GitHub secret `CLOUDEVAL_ACCESS_KEY`, and rerun the workflow. See [Renew an expired access key](docs/github-action.md#renew-an-expired-access-key).
...
| **Line annotations, comments, and SARIF** | Emits GitHub Actions line annotations from source-mapped findings. Set `pr_line_comments: true` to also post bounded PR review comments when a finding has a real changed-file location. For GitHub App-linked projects, line comments are posted by `cloudeval-ai[bot]` through Cloudeval; workflow-token comments are only the fallback path when the App path is not configured. Optional Cloudeval App Check Runs and `review.sarif.json` code scanning upload use the same review evidence. Aggregate gate failures stay in the PR summary instead of being attached to arbitrary lines. |
...
Review PR comments are expanded by default. The visible header separates the configured **Merge gate** result from observed Well-Architected posture, validation, policy checks, and cost budget status. Cloudeval project, report, PDF, workflow, and artifact links render as badges. The `PDF` badge points to the Cloudeval-hosted PDF; when `.cloudeval/config.yaml` enables `ci.review.outputs.pdf.enabled` and `upload_artifacts: true`, the uploaded GitHub artifact also contains `review/review.pdf`, including failed review runs. The AI summary stays visible, while the detailed AI reviewer note folds by default; action queue, Well-Architected, cost, validation, and architecture evidence sections use GitHub-native disclosures. With `emit_annotations: true`, the action emits GitHub Actions line annotations from source-mapped findings in `review.json`. With `pr_line_comments: true`, it posts bounded review comments only when findings map to changed lines, and it replaces stale Cloudeval line comments on reruns. When `project_id` and `CLOUDEVAL_ACCESS_KEY` point to a GitHub App-linked Cloudeval project, those comments are posted by `cloudeval-ai[bot]`; otherwise the action can fall back to workflow-token review comments. If the App endpoint is configured but fails, the action does not post fallback line comments so the author identity stays accurate. When `github_checks: true`, the action also asks the Cloudeval GitHub App to post a native Check Run. When `sarif: true`, the action writes SARIF and exposes `sarif_path` for `github/codeql-action/upload-sarif`.
...
- For PR comments and PR line comments from **forks**, GitHub may block workflow secrets and token permissions; document that for contributors. App-authored comments still require the Cloudeval access key to be available to the workflow.
...
- App-authored PR line comments require the Cloudeval GitHub App to be installed on the repository with **Pull requests: read and write** permission, and the Cloudeval access key must include `github:comment`.
