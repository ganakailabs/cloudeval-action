...
| **Line annotations, comments, and SARIF** | Emits GitHub Actions line annotations from source-mapped findings. Set `pr_line_comments: true` to also post bounded PR review comments when a finding has a real changed-file location. Optional Cloudeval App Check Runs and `review.sarif.json` code scanning upload use the same review evidence. Aggregate gate failures stay in the PR summary instead of being attached to arbitrary lines. |
